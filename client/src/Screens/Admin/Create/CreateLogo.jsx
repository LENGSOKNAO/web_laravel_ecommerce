...
const CreateLogo = () => {
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!displayImage) newErrors.image = "Logo image is recommended";
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response?.status === 422) {
        ...
        setErrors({ general: `Failed to create logo: ${error.message}` });
      }
    } finally {
    ...
    <LayoutAdmin>
      <div className="min-h-screen bg-gray-50">
        <div className="mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="mb-10">
            <h1 className="text-3xl font-bold text-gray-900">Add New Logo</h1>
            <p className="mt-2 text-base text-gray-600">
              Fill out the details to create a new logo
            </p>
          </div>
          {errors.general && (
