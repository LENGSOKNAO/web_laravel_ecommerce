...
const EditSlider = () => {
  ...
  // State management
  const [displayImage, setDisplayImage] = useState(null); // { file, preview } for new upload or { url } for existing
  const [galleryImages, setGalleryImages] = useState([]); // [{ file, preview }] for new or [{ url }] for existing
  ...
  useEffect(() => {
    const fetchSlider = async () => {
      try {
        ...
        console.log("Fetched slider:", slider); // Debug

        setSliderData({
          name: slider.name || "",
          description: slider.description || "",
          date: slider.date ? slider.date.split("T")[0] : "",
        });

        setIsActive(!!slider.status);

        setSelectedCategory(
          slider.category
            ? categoryOptions.find((opt) => opt.value === slider.category) ||
                null
            : null
        );
        ...
        setSelectedBrand(
          slider.brand
            ? brandOptions.find((opt) => opt.value === slider.brand) || null
            : null
        );

        if (slider.image && import.meta.env.VITE_STORAGE_URL) {
          setDisplayImage({
            url: `${import.meta.env.VITE_STORAGE_URL}/${slider.image}`,
          });
        }

        if (slider.small_image && import.meta.env.VITE_STORAGE_URL) {
          setGalleryImages([
            {
              url: `${import.meta.env.VITE_STORAGE_URL}/${slider.small_image}`,
            },
          ]);
        }
        ...
        setErrors({ general: "Failed to load slider data. Please try again." });
        setLoading(false);
      }
    };
    fetchSlider();
  ...
  const handleGalleryImagesChange = (e) => {
    ...
    setGalleryImages(mappedFiles.slice(0, 1)); // Limit to one small image
  ...
  const handleSubmit = async (e) => {
    ...
    if (displayImage?.file) formData.append("image", displayImage.file);
  ...
  if (loading) {
    return (
      <LayoutAdmin>
        <div className="p-8 text-center text-gray-500">Loading slider...</div>
      </LayoutAdmin>
    );
  }
  ...
  if (errors.general) {
    return (
      <LayoutAdmin>
        <div className="p-8 text-center text-red-500">{errors.general}</div>
      </LayoutAdmin>
    );
  }
        ...
        <div className="mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="mb-10">
            ...
            <p className="mt-2 text-base text-gray-600">
              Update the details of the slider
            </p>
          </div>
                        ...
                        <div className="relative w-full">
                          <img
                            ...
                            onLoad={() =>
                              console.log(
                                `Loaded image: ${
                                  displayImage.url || displayImage.preview
                                }`
                              )
                            }
                          />
                            ...
                            {galleryImages.map((image, index) => (
                              <div key={index} className="relative">
                                <img
                                  ...
                                  onError={(e) => {
                                    console.error(
                                      `Failed to load small image: ${e.target.src}`
                                    );
                                    e.target.src =
